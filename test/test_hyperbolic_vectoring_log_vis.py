...
async def test_hyperbolic_basic(dut):
    ...
    plt.plot(s, log_pred, label='Computed log', linestyle='--', color='red')
    plt.legend()

    """"
    plt.subplot(2, 2, 2)
    plt.title("Resdiual : true - predicted")
    plt.plot(s, log_pred - s_pred, label='residue (true - predicted)', color='red')
    plt.legend()
    ...
    plt.savefig(os.path.join(OUTDIR, "log.png"),  dpi=180, bbox_inches="tight")
    plt.close()